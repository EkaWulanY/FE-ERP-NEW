...
<body class="bg-gray-200">
    ...
    <script>
        ...
        let allJobs = []; // simpan semua jobs setelah fetch pertama

        async function fetchActiveJobs() {
            try {
                const response = await fetch('/proxy/jobs');
                allJobs = await response.json(); // simpan ke global
            ...
            try {
                ...
                if (fieldData.data && fieldData.data.length > 0) {
                    fieldData.data.forEach(field => {
                        // FIX: Menggunakan template literal untuk menambahkan HTML
        ...
        function searchJobs() {
            ...
            // filter dari array jobs, bukan DOM
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Auto filter kalau dropdown berubah
